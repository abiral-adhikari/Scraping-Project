...
def select_state(session, state):
    ...
    if response.status_code == 200:
        ...
        if post_response.status_code == 200:
            ...
            return False
    print(f"Failed to load the page for selecting state.")
    return False
...
def select_county(session, county):
    ...
    if response.status_code == 200:
        ...
        if post_response.status_code == 200:
            ...
            return False
    print(f"Failed to load the page for selecting county.")
    return False
...
def accept_disclaimer(session):
    ...
    if response.status_code == 200:
        ...
        if post_response.status_code == 200:
            ...
            return False
    print(f"Failed to load the disclaimer page.")
    return False

# Function to set up the search form with 'Lien' as the document group
def setup_search(session, state, county, start_date, end_date):
    print(f"Setting up search for state: {state}, county: {county}, date range: {start_date} - {end_date}")
    search_url = f"{BASE_URL}/Search.aspx"
    ...
    form_data = {
        ...
        '__EVENTVALIDATION': eventvalidation,
        '__EVENTTARGET': "ctl00$ctl00$MainContent$searchMainContent$ctl01$ctl00$btnSearchDocuments",
        '__EVENTARGUMENT': "",
        ...
        'ctl00$ctl00$MainContent$searchMainContent$ctl01$ctl00$cboDocumentGroup': lien_value,
        'ctl00$ctl00$MainContent$searchMainContent$ctl01$ctl00$btnSearchDocuments': 'Search'
    }
    ...
    if parent_table:
        ...
        if table_content:
            ...
            if main_div:
                ...
                print_results_div = main_div.find("div", id="")
